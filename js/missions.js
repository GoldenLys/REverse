function GenMissions() {
    ...
    for (var M in GLOBALS.MISSIONS) {
        ...
        var REQLEVEL = Game.Level >= GLOBALS.MISSIONS[M][2] ? "<span class='pw green'>" + GLOBALS.MISSIONS[M][2] + "</span>" : "<span class='pw red'>" + GLOBALS.MISSIONS[M][2] + "</span>";
        ...
        if (GLOBALS.MISSIONS[M][2] <= Game.Level && GLOBALS.MISSIONS[M][3] == 2) {
            ...
            if (Game.MissionsCompleted[GLOBALS.MISSIONS[M][9]] == 1 || GLOBALS.MISSIONS[M][9] == -1) {
                let CONTENT = "<h3 class='" + UNLOCKED + "'>" + GLOBALS.MISSIONS[M][0] + "</h3>\
            <div class='ui pw alpha label'> " + FRG + " " + QUALITY + " " + TYPES[GLOBALS.MISSIONS[M][6]] + LEVEL + "</div>\
            " + BTN + "";
...
function hideMissionRewards() {
    if (APP.GOT_REWARDS == 1 && Game.MissionStarted[0]) {
        ...
        if (GLOBALS.MISSIONS[Game.MissionStarted[1]][3] == 2) { Game.FortressesCleared++; Game.Shards += GLOBALS.MISSIONS[Game.MissionStarted[1]][5]; }
        Game.MissionStarted = [false, 0, 0, 0, 0];
        APP.GOT_REWARDS = 0;
    }
    UpdateGame();
